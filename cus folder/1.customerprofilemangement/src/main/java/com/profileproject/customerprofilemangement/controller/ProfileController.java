...
public class ProfileController {
    ...
    @PostMapping()
    public ResponseEntity<ProfileDTO> createCustomer(@RequestBody ProfileDTO profileDTO) {
        ProfileDTO createdProfile = profileService.createCustomer(profileDTO);
        return new ResponseEntity<>(createdProfile, HttpStatus.CREATED);
    }

    @GetMapping()
    public ResponseEntity<List<ProfileDTO>> getAllCustomers() {
        List<ProfileDTO> profiles = profileService.getAllCustomers();
        return ResponseEntity.ok(profiles);
    }

    @GetMapping("/{id}")
    public ResponseEntity<ProfileDTO> getCustomerById(@PathVariable Long id) {
        Optional<ProfileDTO> profile = profileService.getCustomerById(id);
        return profile.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PutMapping("/{id}")
    public ResponseEntity<ProfileDTO> updateCustomer(@PathVariable Long id, @RequestBody ProfileDTO profileDTO) {
        try {
            ProfileDTO updatedProfile = profileService.updateCustomer(id, profileDTO);
